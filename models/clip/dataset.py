...
# class ProcessedCLIPDataset(torch.utils.data.Dataset):
#     def __init__(self, json_path, root_dir, transform=None, tokenizer=None):
#         """
#         Args:
...
#             tokenizer (callable, optional): Function to tokenize text (e.g., clip.tokenize)
#         """
#         with open(json_path, 'r') as f:
#             self.data = json.load(f)
#         self.root_dir = root_dir
...
class CLIPDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        example = self.dataset[idx]
        image = example["image"]
        # Convert only if image mode is not RGB
        if image.mode != "RGB":
            image = image.convert("RGB")
        text = example.get("impression", "") or ""
        return {"image": image, "text": text}
